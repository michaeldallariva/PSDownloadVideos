...
install_ytdlp_binary() {
    ...
    hash -r 2>/dev/null || true   # forget any cached path to an older yt-dlp
    ok "yt-dlp installed."
}

# ============================================
# Ensure a JavaScript runtime (YouTube now requires one)
# yt-dlp's built-in JS interpreter can no longer solve YouTube's challenge,
# so without Deno/Node/Bun/QuickJS you only get storyboard images.
# ============================================
ensure_js_runtime() {
    for rt in deno node bun qjs; do
        command_exists "$rt" && { gray "JS runtime found: $rt"; return 0; }
    done
    # Deno may already be installed but not yet on PATH this session
    export PATH="$HOME/.deno/bin:$PATH"
    command_exists deno && { gray "JS runtime found: deno"; return 0; }

    info "No JavaScript runtime found — installing Deno (required by YouTube for yt-dlp)..."
    if command_exists curl; then
        curl -fsSL https://deno.land/install.sh | DENO_INSTALL="$HOME/.deno" sh -s -- -y >/dev/null 2>&1
    elif command_exists wget; then
        wget -qO- https://deno.land/install.sh | DENO_INSTALL="$HOME/.deno" sh -s -- -y >/dev/null 2>&1
    fi
    export PATH="$HOME/.deno/bin:$PATH"
    ...
    if command_exists deno; then
        ...
        warn "Could not install Deno automatically."
        warn "  Install it manually from https://deno.com — without a JS runtime"
        warn "  YouTube downloads will only return images."
    fi
}
...
# ============================================
# Always use our own fresh binary in ~/.local/bin (first on PATH) so a stale
# distro-packaged yt-dlp — common on Ubuntu — can't shadow it.
if [[ ! -x "$LOCAL_BIN/yt-dlp" ]]; then
    warn ""
    warn "Installing a current yt-dlp to $LOCAL_BIN ..."
...
if ! command_exists yt-dlp; then
    err "yt-dlp still not on PATH. Add $LOCAL_BIN to your PATH and re-run."
    exit 1
fi

# YouTube now requires a JS runtime to solve its challenge (see note below).
ensure_js_runtime
...
if [[ $DOWNLOAD_EXIT -eq 0 ]]; then
    ...
    echo ""
    warn "If you saw \"Only images are available\", YouTube's JS challenge wasn't solved."
    warn "  Make sure a JS runtime is installed and on PATH:  deno --version"
    warn "  Then re-run. You can also try nightly: set USE_NIGHTLY_BUILD=true at the top."
...
# One-time hint: make the tool dirs permanent for future manual yt-dlp use
if [[ ":$PATH:" != *":$HOME/.local/bin:"* ]] || [[ ":$PATH:" != *":$HOME/.deno/bin:"* ]]; then
    echo ""
    gray "Tip: to use yt-dlp/deno directly in new terminals, add this to ~/.bashrc:"
    gray "  export PATH=\"\$HOME/.local/bin:\$HOME/.deno/bin:\$PATH\""
fi
